        ...
        <div className="grid md:grid-cols-3 gap-8">
          <div className="md:col-span-2">
            ...
            <p className="text-muted text-sm mb-1">
              Bachelor of Computer Science (Informatics) · Major Software Engineer
            </p>
            ...
            <div className="flex flex-wrap gap-2">
              {courses.map((c) => (
                <span
                  key={c}
                  className="font-mono text-[11px] px-2.5 py-1 border border-line rounded text-muted bg-surface2"
                >
                  {c}
                </span>
              ))}
          ...
          <div className="border border-line rounded-lg p-5 bg-surface/60 h-fit">
            <div className="font-mono text-xs text-muted mb-3">// gpa.log</div>
            <div className="flex items-end gap-1 h-24">
              {[3.7, 3.85, 3.9, 3.92].map((g, i) => (
                <div key={i} className="flex-1 flex flex-col items-center gap-1">
                  <div
                    className="w-full bg-route/70 rounded-t"
